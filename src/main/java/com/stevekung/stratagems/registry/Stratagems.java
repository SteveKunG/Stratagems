...
public class Stratagems
{
    ...
    {
        register(context, REINFORCE, "wsdaw", Items.RESPAWN_ANCHOR.getDescription(), new ItemStack(Items.RESPAWN_ANCHOR), ReinforceAction.reinforce(), 200, 2400, 20, BLUE_COLOR);
        ...
        register(context, TNT, "swaswdsw", Items.TNT.getDescription(), new ItemStack(Items.TNT), SpawnBombAction.spawnBomb(40), 400, 12000, 3, RED_COLOR);
    }

    static void register(BootstrapContext<Stratagem> context, ResourceKey<Stratagem> key, String code, Component name, ItemStack icon, StratagemAction.Builder action, int incomingDuration, int nextUseCooldown, int remainingUse, int beamColor)
    {
        context.register(key, new Stratagem(code, name, Either.left(icon), action.build(), new StratagemProperties(incomingDuration, Optional.empty(), nextUseCooldown, Optional.of(remainingUse), beamColor)));
    }
    ...
    {
        context.register(key, new Stratagem(code, name, Either.left(icon), action.build(), new StratagemProperties(incomingDuration, Optional.empty(), nextUseCooldown, Optional.empty(), beamColor)));
    }
